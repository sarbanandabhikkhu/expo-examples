...
export default function App() {
  ...
  const importDb = async () => {
    ...
    if (!result.canceled || result.type === "success") {
      ...
      const dbDirectory = await FileSystem.getInfoAsync(FileSystem.documentDirectory + "/SQLite");
      console.log(JSON.stringify(dbDirectory, null, 2));

      if (!dbDirectory.exists) await FileSystem.makeDirectoryAsync(FileSystem.documentDirectory + "/SQLite");

      const base64 = await FileSystem.readAsStringAsync(dbDirectory.uri + "/example.db", {
        encoding: FileSystem.EncodingType.Base64
      });
      console.log(dbDirectory);

      await FileSystem.writeAsStringAsync(dbDirectory.uri + "/example.db", base64, {
        encoding: FileSystem.EncodingType.Base64
      });
